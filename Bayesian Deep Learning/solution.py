...
class SWAInferenceHandler(object):
    ...
    def __init__(
        ...
        self._swag_variance = self._create_weight_copy()
        self._swag_sq_mean = self._create_weight_copy()
        self._n = 0
        ...
        # Full SWAG
        self._deviation_matrices = collections.deque(maxlen=self.max_rank_deviation_matrix)


        # Calibration, prediction, and other attributes
        # TODO(2): create additional attributes, e.g., for calibration
        self._calibration_threshold = 0.8  # this is an example, feel free to be creative
    ...
    def update_swag_statistics(self) -> None:
        ...
        # SWAG-diagonal
        for name, val in copied_params.items():
            # TODO(1): update SWAG-diagonal attributes for weight `name` using `copied_params` and `param`
            old_mean = self._swag_mean[name]
            old_sq_mean = self._swag_sq_mean[name]

            self._swag_mean[name] = old_mean + (val - old_mean) / self._n
            self._swag_sq_mean[name] = old_sq_mean + (val * val - old_sq_mean) / self._n

            
            self._swag_variance[name] = self._swag_sq_mean[name] - self._swag_mean[name] * self._swag_mean[name]

        # Full SWAG
        
        if self.inference_mode == InferenceMode.SWAG_FULL:
            deviation_matrix = {name: (val - self._swag_mean[name]).clone() for name, val in copied_params.items()}
            self._deviation_matrices.append(deviation_matrix)
    ...
    def fit_swag_model(self, loader: torch.utils.data.DataLoader) -> None:
        ...
        for name, _ in self.network.named_parameters():
            self._swag_mean[name].zero_()
            self._swag_sq_mean[name].zero_()
            self._swag_variance[name].zero_()
    ...
    def run_calibration(self, validation_data: torch.utils.data.Dataset) -> None:
        ...
        self._calibration_threshold =0.72
        ...
        for _ in tqdm.trange(self.num_bma_samples, desc="Performing Bayesian model averaging"):
            ...
            with torch.no_grad():
                ...
                for (batch_images,) in loader:
                    ...
                    logits = logits / T  #TTTTT scaling
                    probs = torch.softmax(logits, dim=-1)
                    all_batch_preds.append(probs)

            all_batch_preds = torch.cat(all_batch_preds, dim=0)
            model_predictions.append(all_batch_preds)
    ...
    def sample_parameters(self) -> None:
        ...
        for name, param in self.network.named_parameters():
            ...
            mean_weights = self._swag_mean[name]
            var_weights = self._swag_variance[name]
            std_weights = torch.sqrt(var_weights + 1e-8)

            # Diagonal part
            sampled_weight = mean_weights + std_weights * z_diag

          
            if self.inference_mode == InferenceMode.SWAG_FULL and len(self._deviation_matrices) > 0:
              
                K = self.max_rank_deviation_matrix
                c = 1 / math.sqrt(2 * (K - 1))

                
                deviations = torch.stack([d[name] for d in self._deviation_matrices], dim=0)  
                z2 = torch.randn(K, device=param.device)

               
                low_rank_component = torch.tensordot(z2, deviations, dims=([0], [0]))  

                
                sampled_weight = mean_weights + (1 / math.sqrt(2)) * std_weights * z_diag + c * low_rank_component
...
class SWAGScheduler(torch.optim.lr_scheduler.LRScheduler):
    ...
    def calculate_lr(self, current_epoch: float, previous_lr: float) -> float:
        lr_max = self.optimizer.param_groups[0]['initial_lr']
        lr_min = lr_max * 0.2
        T = self.epochs
        new_lr = lr_min + 0.5 * (lr_max - lr_min) * (1 + math.cos(math.pi * current_epoch / T))
        return new_lr
